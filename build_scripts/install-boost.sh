...
BUILD_DIR=                  # where to clone and build
GXX=/usr/bin/gcc               # if using non-standard gcc set location here
...
VERS_NUM=${CASM_BOOST_VERSION//./_} \
  ...
  && [[ -n "$GXX" ]] && echo "using gcc : 9.3 : /usr/bin/g++-9.3 ;" > tools/build/src/user-config.jam \
  && ./bootstrap.sh \
    --prefix=$PREFIX \
    --with-libraries=filesystem,program_options,regex,chrono,timer,test,system \
